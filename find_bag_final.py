...
def pose_draw(pose):
    ...
    if cx7 == -1 and cx9 != -1:
        cv2.circle(rgb_image, (cx5, cy5), 5, (0, 255, 0), -1)
        ax, ay, az = get_real_xyz(cx5, cy5)

        cv2.circle(rgb_image, (cx9, cy9), 5, (0, 255, 0), -1)
        ...
        cv2.circle(rgb_image, (cx7, cy7), 5, (0, 255, 0), -1)
...
if __name__ == "__main__":    
    ...
    while not rospy.is_shutdown():
        ...
        if step == "start":
            ...
            x, z, _image_show = _fw.calc_cmd_vel(_image1, depth)
            
            _msg_cmd.linear.x = x 
            _msg_cmd.angular.z = z
            _pub_cmd.publish(_msg_cmd)
            
            if "stop" in s:  #add mr_voice
                open_gipper()
                step="back"
        '''
        if step!="follow":
